...
void main(void)
{
  ...
  // Text
   ...
   if(handedness<0.5)
   {
      sizel=2.7*sqrt(cNumberNonGaiaPlayers*playerTiles);
      sizew=2.19*sqrt(cNumberNonGaiaPlayers*playerTiles);
   }
   else
   {
      sizew=2.7*sqrt(cNumberNonGaiaPlayers*playerTiles);
      sizel=2.19*sqrt(cNumberNonGaiaPlayers*playerTiles);
   }
   ...
   // Init map.
   rmTerrainInitialize("cliffPlainB", 12.0);
   ...
   if(huntableNumber<0.2)
      rmAddObjectDefItem(closeHuntableID, "rhinocerous", 6, 2.0);
   else if(huntableNumber<0.4)
      rmAddObjectDefItem(closeHuntableID, "zebra", 8, 2.0);
   else if(huntableNumber<0.6)
      rmAddObjectDefItem(closeHuntableID, "ram", 10, 2.0);
   else if(huntableNumber<0.8)
      rmAddObjectDefItem(closeHuntableID, "elephant", 4, 2.0);
   else
      rmAddObjectDefItem(closeHuntableID, "giraffe", 5, 2.0);
   ...
   // For this map, pick how many zebra in a grouping.  Assign this
   int numHuntable=rmRandInt(6, 16);

   int mediumZebraID=rmCreateObjectDef("medium zebra");
   rmAddObjectDefItem(mediumZebraID, "zebra", numHuntable, 3.0);
   rmSetObjectDefMinDistance(mediumZebraID, 60.0);
   rmSetObjectDefMaxDistance(mediumZebraID, 80.0);
   rmAddObjectDefConstraint(mediumZebraID, shortAvoidSettlement);
   rmAddObjectDefConstraint(mediumZebraID, farStartingSettleConstraint);
   rmAddObjectDefConstraint(mediumZebraID, forestObjConstraint);

   // gold avoids gold, Settlements and TCs
   int farGoldID=rmCreateObjectDef("far gold");
   rmAddObjectDefItem(farGoldID, "Gold mine", 2, 0.0);
/*   rmSetObjectDefMinDistance(farGoldID, 80.0);
   ...
   // goats avoid TCs
   int farCowsID=rmCreateObjectDef("far aurochs");
   rmAddObjectDefItem(farCowsID, "aurochs", 2, 4.0);
   ...
   if(bonusChance<0.3)
      rmAddObjectDefItem(bonusHuntableID, "zebra", 6, 2.0);
   else if(bonusChance<0.6)
      rmAddObjectDefItem(bonusHuntableID, "elephant", 4, 2.0);
   else
      rmAddObjectDefItem(bonusHuntableID, "giraffe", 4, 2.0);
   ...
   if(bonusChance3<0.3)
      rmAddObjectDefItem(bonusHuntableID2, "giraffe", 4, 2.0);
   else if(bonusChance3<0.6)
      rmAddObjectDefItem(bonusHuntableID2, "elephant", 6, 2.0);
   else
      rmAddObjectDefItem(bonusHuntableID2, "rhinocerous", 6, 2.0);
 ...
 // Relics avoid TCs
  ...
  // Text
   ...
   rmAddConnectionTerrainReplacement(connectionID, "cliffNorseA", "GrassA");
   rmAddConnectionTerrainReplacement(connectionID, "cliffNorseB", "GrassA");
   rmAddConnectionTerrainReplacement(connectionID, "cliffGreekA", "GrassA");
...
/*      rmAddAreaConstraint(rockPatch, patchConstraint); */
   ...
   for(i=1; <cNumberPlayers)
   {
      ...
      rmAddAreaTerrainLayer(id, "GrassB", 4, 12);
  ...
  for(i=1; <cNumberPlayers)
   {
      for(j=0; <3)
      {
         ...
         int id3=rmCreateArea("grass patch"+i +j, rmAreaID("player"+i));
   ...
   for(i=1; <cNumberPlayers)
   {
      for(j=0; <3)
      {
         ...
         rmSetAreaTerrainType(id2, "GrassB");
         rmAddAreaTerrainLayer(id2, "PlainB", 0, 2);
...
// Settlements.
   ...
   if(rmPlaceFairLocs())
   {
      ...
      for(i=1; <cNumberPlayers)
      {
         for(j=0; <rmGetNumberFairLocs(i) + 1)
         {
            int settleArea = rmCreateArea("settlement area"+i +j, rmAreaID("Player"+i));
            rmSetAreaTerrainType(settleArea, "PlainA");
   ...
   // Straggler trees.
   rmPlaceObjectDefPerPlayer(stragglerTreeID, false, 2);
